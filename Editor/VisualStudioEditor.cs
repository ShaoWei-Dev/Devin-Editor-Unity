...
namespace Microsoft.Unity.VisualStudio.Editor
{
	...
	public class VisualStudioEditor : IExternalCodeEditor
	{
		...
		private static Dictionary<string, IVisualStudioInstallation> DiscoverInstallations()
		{
			...
			{
				Debug.LogError($"Error detecting Devin installations: {ex}");
		...
		public void OnGUI()
		{
			...
			EditorGUILayout.LabelField("Devin:");
			EditorGUI.indentLevel++;

			var autoCopyRules = EditorGUILayout.Toggle(new GUIContent("Auto copy project rules", "Automatically copy UnityCodeStyleInstructions.md and UnityPerformanceOptimizationInstructions.md to .devin/rules directory when opening a project"), DevinPreferences.AutoCopyProjectRules);
			if (autoCopyRules != DevinPreferences.AutoCopyProjectRules)
				DevinPreferences.AutoCopyProjectRules = autoCopyRules;

			var includeExternal = EditorGUILayout.Toggle(new GUIContent("Include external packages", ""), DevinPreferences.IncludeExternalPackages);
			if (includeExternal != DevinPreferences.IncludeExternalPackages)
				DevinPreferences.IncludeExternalPackages = includeExternal;

			var showLibrary = EditorGUILayout.Toggle(new GUIContent("Show Library folder", ""), DevinPreferences.ShowLibrary);
			if (showLibrary != DevinPreferences.ShowLibrary)
				DevinPreferences.ShowLibrary = showLibrary;
		...
		public bool OpenProject(string path, int line, int column)
		{
			...
			if (!Discovery.TryDiscoverInstallation(editorPath, out var installation)) {
				Debug.LogWarning($"Devin executable {editorPath} is not found. Please change your settings in Edit > Preferences > External Tools.");
				return false;
			}
