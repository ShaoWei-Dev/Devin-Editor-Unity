...
namespace Microsoft.Unity.VisualStudio.Editor
{
	internal class VisualStudioCodeInstallation : VisualStudioInstallation
	{
		...
		{
#if UNITY_EDITOR_OSX
			return Directory.Exists(path) && Regex.IsMatch(path, ".*Devin.*.app$", RegexOptions.IgnoreCase);
#elif UNITY_EDITOR_WIN
			return File.Exists(path) && Regex.IsMatch(path, ".*Devin.*.exe$", RegexOptions.IgnoreCase);
#else
			return File.Exists(path) && path.EndsWith("devin", StringComparison.OrdinalIgnoreCase);
#endif
		}
		...
		public static bool TryDiscoverInstallation(string editorPath, out IVisualStudioInstallation installation)
		{
			...
			installation = new VisualStudioCodeInstallation()
			{
				...
				Name = "Devin" + (isPrerelease ? " - Insider" : string.Empty) + (version != null ? $" [{version.ToString(3)}]" : string.Empty),
				Path = editorPath,
		...
		public static IEnumerable<IVisualStudioInstallation> GetVisualStudioInstallations()
		{
			...
			var envPath = Environment.GetEnvironmentVariable("DEVIN_PATH");
			if (!string.IsNullOrEmpty(envPath))
				candidates.Add(envPath);

			var envExe = Environment.GetEnvironmentVariable("DEVIN_EXE");
			...
			foreach (var basePath in new[] {localAppPath, programFiles})
			{
				...
				candidates.Add(IOPath.Combine(basePath, "Devin Insiders", "Devin - Insiders.exe"));
			}
#elif UNITY_EDITOR_OSX
		...
		private static IEnumerable<string> GetXdgCandidates()
		{
			...
			foreach(var dir in dirs)
			{
				...
				try
				{
					var desktopFile = IOPath.Combine(dir, "applications/devin.desktop");
    ...
    ""configurations"": [
		...
		private void CreateSettingsFile(string vscodeDirectory, bool enablePatch)
		{
			...
			var libraryExcludes = DevinPreferences.ShowLibrary ? string.Empty : @"        ""Library/"": true,
        ""library/"": true,
";
    ...
    ""dotnet.defaultSolution"": """ + IOPath.GetFileName(ProjectGenerator.SolutionFile()) + @"""
		...
		private void PatchSettingsFile(string settingsFile)
		{
			try
			{
				...
				if (DevinPreferences.ShowLibrary)
				{
...
";
		...
		public override bool Open(string path, int line, int column, string solution)
		{
			...
			{
				if (DevinPreferences.AutoCopyProjectRules)
				{
					EnsureProjectRules(projectRoot);
				}
			}

			if (DevinPreferences.IncludeExternalPackages && !string.IsNullOrEmpty(absolutePath) && !string.IsNullOrEmpty(projectRoot) && !IsPathUnderDirectory(absolutePath, projectRoot))
			{
				...
				if (!string.IsNullOrEmpty(externalPackageRoot))
				{
					...
					var workspaceFile = EnsureDevinWorkspace(projectRoot, projectName, externalPackageRoot, solutionFileName);
		...
		private static void EnsureProjectRules(string projectRoot)
		{
			try
			{
				var rulesDirectory = IOPath.Combine(projectRoot, ".devin", "rules");
		...
		private static string EnsureDevinWorkspace(string projectRoot, string projectName, string externalPackageRoot, string solutionFileName)
		{
			...
			try
			{
				...
				var workspaceFile = IOPath.Combine(projectRoot, $"{safeProjectName}.devin.code-workspace");
