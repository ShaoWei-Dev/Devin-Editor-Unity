...
namespace Microsoft.Unity.VisualStudio.Editor
{
	internal static class Cli
	{
		internal static void Log(string message)
		{
			...
			Console.WriteLine($"[Devin.Editor.{nameof(Cli)}] {message}");
		}
		...
		internal static void GenerateSolutionWith(VisualStudioEditor vse, string installationPath)
		{
			...
			{
				Log($"No Devin installation found in ${installationPath}!");
			}
		}
		...
		internal static void GenerateSolution()
		{
			...
			{
				Log($"Using default editor settings for Devin installation");
				GenerateSolutionWith(vse, CodeEditor.CurrentEditorInstallation);
			}
			else
			{
				Log($"Devin is not set as your default editor, looking for installations");
				try
				{
					...
					{
						Log($"No Devin installation found!");
					}
				}
				catch (Exception ex)
				{
					Log($"Error detecting Devin installations: {ex}");
				}
			}
		}
	}
